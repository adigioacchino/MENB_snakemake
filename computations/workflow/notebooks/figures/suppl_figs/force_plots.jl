...
# ╔═╡ 314a5530-5d61-11ee-22b8-d37a960d624b
begin
	using Statistics
	using CairoMakie
	...
	using MaxEntNucleotideBiases: NucleotideModel, readmodel
end

# ╔═╡ 3ffb8ac3-1c8a-4c30-a439-9e25f786886d
# prepare data for plotting
begin
	fams = ["Orthomyxo", "Flavi", "Corona", "Picorna"]
	
	fixed_hosts = nothing
	fixed_motifs = nothing
	virhostmodelss = Dict{String, Dict{String, Tuple{Vector{Float64}, Vector{Float64}}}}()
	
	for fam in fams
	    dir = "..\\..\\..\\..\\results\\models\\"
	    fnames = readdir(dir)
	    c1 = occursin.(fam, fnames)
	    c2 = occursin.("half", fnames)
	    virmodels = fnames[c1 .& c2]
	    ...
	    for host in unique(getindex.(split.(virmodels, "_"), 2))
	        ...
	        mods = readmodel.(joinpath.(dir, virmodels[c3]))
	        forces = [vcat([m.forces[length.(m.motifs) .== l] for l in 1:3]...) for m in mods]
	        virhostmodels[host] = (mean(forces), std(forces))
	    end
	
	    virhostmodelss[fam] = virhostmodels
	
	    model = readmodel.(joinpath.(dir, virmodels[1]))
	    motifs = vcat([model.motifs[length.(model.motifs) .== l] for l in 1:3]...)
	    if fixed_motifs == nothing
	        global fixed_motifs = motifs
	    else
	        @assert fixed_motifs == motifs
	    end
	
	    if fixed_hosts == nothing
	        global fixed_hosts = hosts
	    else
	        @assert fixed_hosts == hosts
	    end
	end
end;

# ╔═╡ 01d7bba7-beed-4b4a-bc3c-421f749e7090
# parameter settings
savefig = false;

# ╔═╡ b1da1ef7-8dfc-496f-9378-e90333b90dea
# plot
begin
	fig = Figure(resolution = (1200, 450*4), fontsize = 24)
	...
	for (i, fam) in enumerate(fams)
	    hosts = fixed_hosts
	    motifs = fixed_motifs
	    virhostmodels = virhostmodelss[fam]
	...
	#############################################################################################################
	#############################################################################################################
	savefig && save("supplfig_allforces.pdf", fig)
	current_figure()
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "2f0d0ae5daa7f9d5a2bb21bff4a6c4a7299bfc9c"
