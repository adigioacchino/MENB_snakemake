...
# ╔═╡ 0c2f2ef3-af0d-49fe-8f6a-dd958b45e992
begin
	using CairoMakie 
	using PlutoUI
	...
	using MaxEntNucleotideBiases: NucleotideModel, readmodel, compute_symmetrized_kl
end

# ╔═╡ 2b0b4430-5303-11ee-2c88-bd4cee5a4cc7
md"""
Here I produce a plot of the sKL matrix between all (host,virus) pairs (so 12 x 12).
"""

# ╔═╡ 17a3c157-44d0-4e89-8df1-57aab91507ea
# load models
begin
	viruses = ["Corona", "Picorna", "Flavi", "Orthomyxo"]
	hosts = ["avian", "human", "swine"]
	seed_used = "1"

	model_path = "..\\..\\..\\..\\results\\models"
	lastpart = "NucleotideModel_full_$(seed_used).txt"
	models = Dict([(v, h) => readmodel(joinpath(model_path, "$(v)_$(h)_$(lastpart)")) for v in viruses for h in hosts])	
end;

# ╔═╡ 5b7e0584-3323-479f-9a56-ae5251905691
# choose sorting of pairs
sorted_pairs = [(v, h) for v in viruses for h in hosts];

# ╔═╡ 12d72843-0fae-429f-b136-eca66e5ec287
# compute KL divergences
begin
	KL_matrix = [compute_symmetrized_kl(1000, models[x], models[y], fast=true) for x in sorted_pairs, y in sorted_pairs]
end;

# ╔═╡ 27772e60-2ce1-4b93-bb36-f2bc6a224e54
md"""
res x = $(@bind res_x PlutoUI.Slider(400:50:1200, default=850, show_value=true))

res y = $(@bind res_y PlutoUI.Slider(400:50:1200, default=750, show_value=true))

savefig = $(@bind savefig CheckBox())

"""

# ╔═╡ 87ac83fa-c816-43a0-b9e0-a2e6b2e0a6ba
# plot matrix
begin
	fig = Figure(resolution = (res_x, res_y), fontsize = 24)
	xax_labels = ["$(x[1])-$(x[2])" for x in sorted_pairs]
	yax_labels = ["$(x[1])-$(x[2])" for x in sorted_pairs]
	ax1 = Axis(fig[1, 1],
	           xticks = (1:length(xax_labels), xax_labels), xticklabelrotation=pi/3,
	           yticks = (1:length(yax_labels), yax_labels),
			   title = "symmetrized KL divergence")
	
	hm = heatmap!(ax1, 1:length(xax_labels), 1:length(yax_labels), KL_matrix, colormap = :thermal)
	
	Colorbar(fig[:, end+1], hm)
	
	resize_to_layout!(fig)
	savefig && save("KL_table.pdf", fig)
	current_figure()
end
...
julia_version = "1.9.3"
manifest_format = "2.0"
project_hash = "80cb7568373ef02cfd7375719f11d98ed24afe81"
