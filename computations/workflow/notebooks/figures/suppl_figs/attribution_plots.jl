...
# ╔═╡ b61f43e0-5d3b-11ee-0be0-7921054f7dab
begin
...
# ╔═╡ fc2e9525-9ad7-4fbb-956c-5b59dbae2905
...
# ╔═╡ bd37d58a-9bcb-4ed2-96ef-912cb8443fea
begin
	seed = 1
	model_path = "../../../../results/models/"
	attr_path = "../pre_computations"
	mkpath(attr_path)
	
	fams = ["Orthomyxo", "Flavi", "Corona", "Picorna"]
	pairs = [["human", "avian"], ["human", "swine"], ["swine", "avian"]]
	
	sKL_atts = Dict{String, Dict{String, Vector{Tuple{String, Float64}}}}()
	
	for fam in fams
	    t_atts = Dict{String, Vector{Tuple{String, Float64}}}()
	    for pair in pairs
	        ...
	        # Compute sKL attributions
	        filename = "sKL_attributions_$(fam)_$(pair[1])$(pair[2])_half.h5"
	        if filename in readdir(attr_path)
	            # read from HDF5
	            t_atts[pair[1]*pair[2]] = h5open(joinpath(attr_path, "sKL_attributions_$(fam)_$(pair[1])$(pair[2])_half.h5"), "r") do h5
	                nts = h5["nts"][:]
	                attrs = h5["attrs"][:]
	            ...
	            println("Working on family: $(fam); pair: ($(pair[1]), $(pair[2]))...")
	            t_atts[pair[1]*pair[2]] = sKL_attributions(mod1, mod2)
	            # save as HDF5
	            h5open(joinpath(attr_path, "sKL_attributions_$(fam)_$(pair[1])$(pair[2])_half.h5"), "w") do h5
	                h5["nts"] = first.(t_atts[pair[1]*pair[2]])
	                h5["attrs"] = last.(t_atts[pair[1]*pair[2]])
	            end
	        end
	    end
	    sKL_atts[fam] = t_atts
...
# ╔═╡ 4b41abcd-3d55-4f5e-9dd8-f3b9f4d5a0db
# figure settings
savefig = false

# ╔═╡ 8c77494e-8b4f-441c-b2a5-7fe6b370d11a
# plot
begin
	fig = Figure(resolution = (1200, 300*6), fontsize = 26)
	...
	for (i, fam) in enumerate(fams)
	    for (j, pair) in enumerate(pairs)
	        t_sKL_att = sKL_atts[fam][pair[1]*pair[2]]
	        topM_attr_poss = sortperm(abs.(getindex.(t_sKL_att,2)), rev=true)[1:M] 
	        topM_attr_signs = sign.(getindex.(t_sKL_att,2))[topM_attr_poss]
	        topM_attr_vals = abs.(getindex.(t_sKL_att,2))[topM_attr_poss]
	        topM_attr_mots = getindex.(t_sKL_att,1)[topM_attr_poss]
	
	        ax = Axis(fig[printcounter, div(i-1, 2)+1], ylabel="abs sKL attribution\n$(fam),\n$(pair[1]) - $(pair[2])",
	                    xticks = (1:M, topM_attr_mots), xticklabelrotation=pi/2)
	        printcounter == 6 ? printcounter = 1 : printcounter += 1
	        
	        c1, c2 = Makie.wong_colors()[1:2]
	        colors = [topM_attr_signs[k] > 0 ? c1 : c2 for k in 1:M]
	        barplot!(ax, topM_attr_vals, color= colors, bar_width = 0.8, strokewidth = 2)
	    end
	end
	#############################################################################################################
	#############################################################################################################
	savefig && save("attribtion_plots.pdf", fig)
	current_figure()
end
